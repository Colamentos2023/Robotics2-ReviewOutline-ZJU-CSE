...
\begin{enumerate}
    ...
    \item \textbf{基于位置识别的定位}
        \begin{itemize}
            ...
            \item \textbf{应用约束}：
            \begin{itemize}
                \item 环境的\textbf{动态变化}和不同位置的\textbf{相似性}，对位置辨别带来挑战
                \item 为适应环境的动态变化，可以考虑对每个位置存储\textbf{多张图片}，但增加存储空间需求
...
\newpage
\subsection*{概统知识回顾}
在正式开始对马尔可夫自定位的介绍之前，我们最好回顾一下相关概率论与数理统计的知识基础；本部分是我自己加的\footnote{本节参考自 ZJUDancer 视觉组内训：\href{https://www.bilibili.com/video/BV1wUmPYCERx/?spm_id_from=333.337.search-card.all.click&vd_source=5deca47094767260f853b759a625a68f}{B 站视频}}，对此感到熟悉的同学可以直接跳过到\hyperref[sec:markov]{控制感知信息融合的自定位}继续阅读。
    \begin{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item \textbf{绑架问题（kidnapped robot problem）}：机器人在定位良好的条件下突然被移到另一个未被告知的地方\footnote{与全局定位问题的差异在于，当机器人被绑架时，机器人可能已经对自已所在的位置非常确信了}，用于检测定位算法发现错误并从错误中恢复的能力
    \end{enumerate}
    ...
    \item \textbf{概率描述}：定位问题可以定义为\textbf{最优估计问题}，最优估计的一个体现是估计的方差很小——用贝叶斯估计器\textbf{是一种最小方差估计器}\\
        ...
        \]
        即在给定当前时刻以及历史所有观测$\mathbf{Z}^{t}$、上一时刻以及之前所有控制$\mathbf{U}^{t-1}$，以及地图$\mathbf{m}$后，路径的$\mathrm{x}^t$最优估计。\label{prove}
        {\small\kaishu 
        \begin{enumerate}
        \item \textbf{问题定义}：\\
        记地图为 \( \mathrm{m} \) ,\\
...
\subsection{马尔可夫定位中的运动模型}
        ...
        \begin{itemize}
            ...
            \end{figure}
            {\small\kaishu 
            $$ p\left( {{\mathrm{x}}_{t} \mid  {\mathrm{x}}_{t - 1},{\mathbf{u}}_{t - 1},\mathbf{m}}\right)  = p\left( {\mathbf{m} \mid  {\mathrm{x}}_{t},{\mathrm{x}}_{t - 1},{\mathbf{u}}_{t - 1}}\right) p\left( {{\mathrm{x}}_{t} \mid  {\mathrm{x}}_{t - 1},{\mathbf{u}}_{t - 1}}\right)$$
            地图分为可通行和不可通行\\
        ...
        \end{itemize}\footnote{这里$x'$就是$x_{t}$,$x$就是$x_{t-1}$，对于$y、\theta$同理}
\label{yundongmoxing}
\begin{enumerate}
    \item \textbf{里程计运动模型}\\
        ...
        \( \left( \begin{array}{l} {x}^{\prime } \\  {y}^{\prime } \\  {\theta }^{\prime } \end{array}\right)  = \left( \begin{matrix} x + {\widehat{\delta }}_{\text{trans }}\cos \left( {\theta  + {\widehat{\delta }}_{rot1}}\right) \\  y + {\widehat{\delta }}_{\text{trans }}\sin \left( {\theta  + {\widehat{\delta }}_{rot1}}\right) \\  \theta  + {\widehat{\delta }}_{rot1} + {\widehat{\delta }}_{rot2} \end{matrix}\right) \)
         ...
         真实值与测量值之间的误差应服从该分布“误差越小，概率越高；误差越大，概率越低”，我们可以设定误差为均值为0的高斯分布
            \begin{figure}[H]
                ...
                \caption{设定为高斯分布的误差}
            \end{figure}
            $$ p( \widetilde{\delta }_{rot1})  = N\left( {{\widetilde{\delta }}_{{rot}1};0,{\alpha }_{1}\left| {\delta }_{{rot}1}\right|  + {\alpha }_{2}\left| {\delta }_{trans}\right| }\right) $$
            ...
            $$ p\left( {\widetilde{\delta }}_{rot2}\right)  = N\left( {{\widetilde{\delta }}_{rot2};0,{\alpha }_{5}\left| {\delta }_{rot2}\right|  + {\alpha }_{6}\left| {\delta }_{trans}\right| }\right) $$
                        {\small\kaishu 
