...
\begin{enumerate}
    ...
    \\{\small\kaishu （只考虑工作空间的几何约束，不考虑机器人的运动学模型和约束，将移动机器人简化为工作空间中的一个点）}
    ...
    \begin{itemize}
        ...
        \item \textbf{位形空间}：机器人成为一个可移动点，不考虑姿态、体积和非完整运动学约束，包括：
            \begin{itemize}
                ...
                \item \textbf{自由空间}：可行的位形集合。{\small\kaishu （在该空间中，机器人将无碰地安全移动。路径规划就是在自由位形空间中为机器人寻找一条路径，使其从初始位置运行到目标位置）}
            \end{itemize}
    \end{itemize}
    \begin{figure}[H]
        \centering
    ...
    \item \textbf{算法}：
        \begin{itemize}
            \item \textbf{完备性的概念}：当解存在时，能够在\textbf{有限的时间}内找到解。
    ...
    \begin{itemize}
        ...
        \\ {\small\kaishu 当选择下一个探索结点时，通过启发式评价函数进行评估，选择路径代价最小的结点作为下一步探索结点而跳转其上。}
            \begin{itemize}
                ...
                \end{itemize}
            \end{itemize}
        \item \textbf{路径成本的计算（$h(n)$的选取）}：欧氏距离或曼哈顿距离，当$h(n)$可采纳\footnote{即$h)(n)<\text{真实最小距离}h^*(n)$，此时可采用，一定能找到最优解；否则将过分估计，不一定找到最优解。换言之，可采用的$h(n)$总是乐观的。}时最优。
        \begin{figure}[H]
            \centering
...
\begin{table}[H]
    ...
    \begin{tabular}{cccccc}
        ...
        A*算法 & 1 & 1$^\ast$ & 1 & 1 & 综合代价与启发式：$f=g+h$\\
        贪心算法 & 0 & 0 & 0 & 1 & 仅考虑启发函数$h(n)$\\
        可调贪婪算法 & 0$\sim$1 & 0$\sim$1 & $a\in[0,1)$ & $b\in(0,1]$ & $f=g+\epsilon h$\\
        \bottomrule
    \end{tabular}
...
\begin{enumerate}
    ...
    \item \textbf{RRT}\label{item:prob:rrt}
        ...
        \begin{itemize}
            ...
            \item \textbf{影响规划收敛速度的三个步骤}
                ...
                \end{itemize}
        \end{itemize}
            \item \textbf{优点}：致力于找到从起点到终点的路径，相比PRM更具目标导向性劣势\\
            \item \textbf{缺点}：产生的路径非最优不够高效
        \begin{enumerate}
    ...
    \begin{tabular}{ccc}
      \toprule
      \textbf{算法} & \textbf{概率完备性} & \textbf{渐进最优性} \\
      \midrule
      PRM  & 1 & 0 \\
      RRT  & 1 & 0 \\
      RRT* & 1 & 1 \\
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/bijiao.png}
    \caption{PRM、RRT的概率完备性如何实现}
\end{figure}
\end{document}