...
\begin{enumerate}
  ...
  $$
          计算$\mathbf{K}_t$时，时间复杂度是$O(k^{2.376})$，其中$k$是特征向量维度，即观测到的特征数\\
          计算$\boldsymbol{\Sigma}_t$时，时间复杂度是$O(n^2)$，其中$n$是状态向量维度，即机器人的位姿描述维度
  \item \textbf{推导过程}\label{ekf_process}
  ...
  \end{enumerate}
    ...
    \end{figure}
        ...
        \end{align}
        则积分内是两个高斯分布，即两个高斯分布的卷积：接下来我们只需要算出这两个高斯分布的均值和方差即可。\\
...
\begin{enumerate}
    \item \textbf{蒙特卡洛法（Monte Carlo Method）}
        ...
        \end{itemize}
            ...
            $$ p\left( {{\mathbf{Z}}_{t} \mid  {\mathbf{Z}}_{t - 1},{\mathbf{U}}_{t-1} ,\mathbf{m}}\right)  \approx  \textcolor{red}{\mathop{\sum }\limits_{{{t}^{\prime } = {t_1}}}^{t}}\frac{1}{M}\mathop{\sum }\limits_{{m = 1}}^{M}{\omega }_{{t}^{\prime }}^{\left\lbrack  m\right\rbrack  } $$
