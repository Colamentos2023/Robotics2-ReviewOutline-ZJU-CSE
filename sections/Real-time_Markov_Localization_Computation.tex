...
\begin{enumerate}
  ...
  \end{enumerate}
    ...
    \end{figure}
        ...
        $$\approx \arg\max_{c_t^i} \mathscr{N} \left( \mathbf{z}_t^i; h(\bar{\mathbf{\mu}}_t, c_t^i, \mathbf{m}), \mathbf{H}_t^i \bar{\boldsymbol{\Sigma}}_t \mathbf{H}_t^{iT} + \mathbf{Q}_t \right)$$
        \textbf{推导过程}：
        {\small\kaishu{
        $$
        p(\mathbf{z}_t^i | C^t, \mathbf{m}, \mathbf{Z}^{t-1}, \mathbf{U}^{t-1})$$
        由于$\mathbf{x}_t$与$\mathbf{z}_t$相关，但是在条件里却没有，所以我们得把这个隐变量弄出来，写成二者的联合概率分布；实际上原式就等于下面这个联合概率分布对$\mathbf{x_t}$积分：
        $$p(\mathbf{z}_t^i | C^t, \mathbf{m}, \mathbf{Z}^{t-1}, \mathbf{U}^{t-1})=\int p(\mathbf{z}_t^i,\mathbf{x}_t | C^t, \mathbf{m}, \mathbf{Z}^{t-1}, \mathbf{U}^{t-1})\mathrm{d}\mathbf{x}_t$$   
        使用乘法规则：
        $$p(\mathbf{z}_t^i | C^t, \mathbf{m}, \mathbf{Z}^{t-1}, \mathbf{U}^{t-1})=\int p(\mathbf{z}_t^i |\mathbf{x}_t,C^t, \mathbf{m}, \mathbf{Z}^{t-1}, \mathbf{U}^{t-1})p(\mathbf{x}_t |C^t, \mathbf{m}, \mathbf{Z}^{t-1}, \mathbf{U}^{t-1})\mathrm{d}\mathbf{x}_t$$  
        则积分内是两个高斯分布，即两个高斯分布的卷积：
        \begin{align}
        p(\mathbf{z}_t^i \mid C^t,\mathbf{m},\mathbf{Z}^{t-1},\mathbf{U}^{t-1})
        &= \int p(\mathbf{z}_t^i,\mathbf{x}_t \mid C^t,\mathbf{m},\mathbf{Z}^{t-1},\mathbf{U}^{t-1})\,\mathrm{d}\mathbf{x}_t \\
        &= \int p(\mathbf{z}_t^i \mid \mathbf{x}_t,C^t,\mathbf{m})\; p(\mathbf{x}_t \mid \mathbf{m},\mathbf{Z}^{t-1},\mathbf{U}^{t-1})\,\mathrm{d}\mathbf{x}_t .
        \end{align}
        接下来我们只需要算出这两个高斯分布的均值和方差即可。\\
        \noindent\textbf{两项分别为高斯：}
\[
p(\mathbf{x}_t \mid \mathbf{Z}^{t-1},\mathbf{U}^{t-1})
=\mathcal{N}\!\big(\mathbf{x}_t;\ \bar{\boldsymbol{\mu}}_t,\ \bar{\boldsymbol{\Sigma}}_t\big),
\qquad
p(\mathbf{z}_t^{\,i}\mid \mathbf{x}_t,c_t^i,\mathbf{m})
\approx \mathcal{N}\!\Big(\mathbf{z}_t^{\,i};\ \mathbf{h}_t^{\,i} + \mathbf{H}_t^{\,i}\!\big(\mathbf{x}_t-\bar{\boldsymbol{\mu}}_t\big),\ \mathbf{Q}_t\Big),
\]
其中
\[
\mathbf{h}_t^{\,i} \;=\; h\!\big(\bar{\boldsymbol{\mu}}_t,\ c_t^i,\ \mathbf{m}\big), 
\qquad 
\mathbf{H}_t^{\,i} \;=\; \left.\dfrac{\partial h(\mathbf{x}_t,\ c_t^i,\ \mathbf{m})}{\partial \mathbf{x}_t}\right|_{\mathbf{x}_t=\bar{\boldsymbol{\mu}}_t}.
\]

\noindent\textbf{写成线性—高斯形式：}
\[
\mathbf{z}_t^{\,i}
= \underbrace{\big(\mathbf{h}_t^{\,i} - \mathbf{H}_t^{\,i}\,\bar{\boldsymbol{\mu}}_t\big)}_{\displaystyle \mathbf{b}_t^{\,i}}
\;+\; \mathbf{H}_t^{\,i}\,\mathbf{x}_t \;+\; \boldsymbol{\omega}_t,
\quad \boldsymbol{\omega}_t \sim \mathcal{N}(\mathbf{0},\mathbf{Q}_t),\quad
\mathbf{x}_t \sim \mathcal{N}(\bar{\boldsymbol{\mu}}_t,\bar{\boldsymbol{\Sigma}}_t).
\]

\noindent\textbf{线性高斯变换的矩（这里我们直接给出）：}
\[
\mathbb{E}[\mathbf{z}_t^{\,i}]
= \mathbf{b}_t^{\,i} + \mathbf{H}_t^{\,i}\,\bar{\boldsymbol{\mu}}_t
= \mathbf{h}_t^{\,i},
\qquad
\mathrm{Cov}[\mathbf{z}_t^{\,i}]
= \mathbf{H}_t^{\,i}\,\bar{\boldsymbol{\Sigma}}_t\,(\mathbf{H}_t^{\,i})^\top + \mathbf{Q}_t
\;\; \triangleq\;\; \mathbf{S}_t^{\,i}.
\]

\noindent\textbf{从而积分结果（两个高斯的卷积）为：}
\[
p(\mathbf{z}_t^{\,i} \mid C^t,\mathbf{m},\mathbf{Z}^{t-1},\mathbf{U}^{t-1})
= \int p(\mathbf{z}_t^{\,i}\mid \mathbf{x}_t,c_t^i,\mathbf{m})\;
        p(\mathbf{x}_t\mid \mathbf{Z}^{t-1},\mathbf{U}^{t-1})\; \mathrm{d}\mathbf{x}_t
= \mathcal{N}\!\big(\mathbf{z}_t^{\,i};\ \mathbf{h}_t^{\,i},\ \mathbf{S}_t^{\,i}\big).
\]

\noindent\textbf{代入最大似然数据关联即得：}
\[
\hat{c}_t^{\,i}
= \arg\max_{c_t^{\,i}}
\; \mathcal{N}\!\Big(
\mathbf{z}_t^{\,i};\ h\!\big(\bar{\boldsymbol{\mu}}_t,\ c_t^{\,i},\ \mathbf{m}\big),
\ \mathbf{H}_t^{\,i}\,\bar{\boldsymbol{\Sigma}}_t\,(\mathbf{H}_t^{\,i})^\top + \mathbf{Q}_t
\Big).
\]
    
    
        %请您作为移动机器人领域的专家，以最严谨、最准确、最认真的态度，完成证明推导过程，言简意赅，只包含必要公式和文字，最简证明但是逻辑必须清晰、推导必须完整，最好参考前文的内容的符号和说明，或者直接用前文中的推导过程这样更好，使得读者更容易理解，分别求两个高斯分布的均值和方差，最后合到一起%
        }}
...
\begin{enumerate}
    \item \textbf{蒙特卡洛法（Monte Carlo Method）}
        ...
        \item \textbf{采样}：
            ...
            \item \textbf{重采样}
                \begin{itemize}
                    ...
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.5\linewidth]{images/fenbu1.png}
                    ...
                    之后按照权重重新采样即可。这里是把路标而不是运动模型作为建议分布，后面建议分布常用运动模型。
                    \begin{figure}[H]
                        \centering
        ...
        \begin{itemize}
            ...
            \item \textbf{PF算法中的误差}：
                \begin{enumerate}
                    ...
                    \item \textbf{由随机采样带来的偏差}
                        \begin{itemize}
                            ...
                            \item 由随机采样所带来的偏差称为采样器方差
                            \begin{figure}[H]
                                \centering
                                \includegraphics[width=0.5\linewidth]{images/sample_error2.png}
                                \caption{随机采样带来误差}
                            \end{figure}
                        \end{itemize}
                ...
                \begin{itemize}
                    ...
                    \item 无论是否重采样，\textbf{保持权重估计}\footnote{权重估计仍然在做，但是做完之后不一定重采样了，如果不重采样的话，权重与上次的权重不断叠加直到下一次重采样后再全都均匀分布}
        ...
        \end{itemize}
            ...
            $$ p\left( {{\mathbf{Z}}_{t} \mid  {\mathbf{Z}}^{t - 1},{\mathbf{u}}_{{1} : t - 1},\mathbf{m}}\right)  \approx  \textcolor{red}{\mathop{\sum }\limits_{{{t}^{\prime } = {t_1}}}}\frac{1}{M}\mathop{\sum }\limits_{{m = 1}}^{M}{\omega }_{{t}^{\prime }}^{\left\lbrack  m\right\rbrack  } $$
            ...
            \textbf{例题}：\\
                ...
                解：\\
                (a)由题意。建议分布这里用观测模型而不是运动模型，分别为$$p(\mathbf{x}_t|\mathbf{z}_1),p(\mathbf{x}_t|\mathbf{z}_2),p(\mathbf{x}_t|\mathbf{z}_3)$$
                目标分布是：$$p(\mathbf{x}_t|\mathbf{z}_1,\mathbf{z}_2,\mathbf{z}_3)$$
                则权重计算与前面的理论部分相同。
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.6\linewidth]{images/liti3.png}
                    \caption{粒子滤波例题(a)，蓝色的是建议分布在这里画粒子即可}
                \end{figure}
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.6\linewidth]{images/liti4.png}
                    \caption{粒子滤波例题(a)，重采样后粒子分布集中在红色区域}
                \end{figure}
                (b)同理，但是扩展到四个对称方向上，这里直接给出重采样后的分布：
                \begin{figure}[H]
                    \centering
                    \includegraphics[width=0.6\linewidth]{images/liti5.png}
                    \caption{粒子滤波例题(b)，重采样后粒子分布集中在四个圆形区域}
                \end{figure}
                }}
